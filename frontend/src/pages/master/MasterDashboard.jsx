  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={ShoppingBag} label="Orders" value={(totalOrders || 0).toLocaleString()} sub={`Period: ${period}`} color="text-green-400" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-dark-700/50">
              <tr>
                {['Order #', 'Franchise', 'Customer', 'Amount', 'Payment', 'Status', 'Time'].map(h => (
            ...
            <tbody>
              {(recentOrders || []).map((order) => (
                <tr key={order._id} className="table-row">
                  <td className="table-cell font-mono text-brand-400">{order.order_number}</td>
                  <td className="table-cell">{order.franchise_id?.franchiseCode}</td>
                  <td className="table-cell">{order.customer_id?.name}</td>
                  <td className="table-cell font-mono">₹{order.final_amount?.toLocaleString('en-IN')}</td>
                  ...
                  <td className="table-cell">
                    <span className={`badge status-${order.kitchen_status?.toLowerCase()}`}>{order.kitchen_status}</span>
                  </td>
