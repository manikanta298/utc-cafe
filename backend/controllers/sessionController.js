...
module.exports = { startSession, addOrderToSession, getSession, generateBill, recordPayment, getSessions, linkCustomer, holdSession, resumeSession, getHeldSessions, cancelSession, approveCancelSession, rejectCancelSession, clearOrderItems, removeItemFromKitchen, removeSessionItem };

// ── Secure delete of a saved session item (requires Franchise Owner PIN) ──
async function removeSessionItem(req, res) {
  try {
    const { itemId, pin, reason } = req.body;
    if (!itemId || !pin) {
      return res.status(400).json({ success: false, message: 'itemId and pin are required' });
    }

    const session = await OrderSession.findById(req.params.sessionId).populate('franchiseId');
    if (!session) {
      return res.status(404).json({ success: false, message: 'Session not found' });
    }

    // Franchise isolation check. Never trust a franchise ID supplied by the browser.
    const userFranchise = (req.user.franchise_id?._id || req.user.franchise_id)?.toString();
    const sessFranchise = (session.franchiseId?._id || session.franchiseId)?.toString();
    if (req.user.role !== 'master_admin' && userFranchise !== sessFranchise) {
      return res.status(403).json({ success: false, message: 'Access denied' });
    }
    ...
    if (!franchise?.edit_pin) {
      return res.status(400).json({ success: false, message: 'No edit PIN configured for this franchise' });
    }

    // Verify the PIN again on the destructive endpoint. The UI modal is not an authorization boundary.
    const pinMatch = await bcrypt.compare(String(pin), franchise.edit_pin);
    if (!pinMatch) {
      return res.status(401).json({ success: false, message: 'Incorrect PIN' });
    }

    // Find the exact embedded session item by its own _id.
    let foundSubOrder = null;
    let foundItem = null;
    for (const sub of session.subOrders || []) {
      const item = sub.items?.id?.(itemId);
      if (item) {
        foundSubOrder = sub;
        foundItem = item;
        break;
    ...
    if (!foundItem || !foundSubOrder) {
      return res.status(404).json({ success: false, message: 'Item not found in session' });
    }

    const itemName = foundItem.name;
    const itemQty = Number(foundItem.qty || 0);
    const itemAmount = Number(foundItem.totalPrice || (foundItem.unitPrice * foundItem.qty) || 0);

    // Find the corresponding Order document before mutating the session. The Order item has
    // its own _id, so matching by the session item's _id is incorrect. Match on the linked
    // order, menu item, quantity and snapshot amount within the same addition round.
    let linkedOrder = null;
    let linkedOrderItemIndex = -1;
    if (foundSubOrder.order_id) {
      linkedOrder = await Order.findById(foundSubOrder.order_id);
      if (linkedOrder) {
        linkedOrderItemIndex = linkedOrder.items.findIndex((orderItem) =>
          orderItem.item_id?.toString() === foundItem.menuItemId?.toString() &&
          Number(orderItem.quantity) === itemQty &&
          Math.abs(Number(orderItem.item_total || 0) - itemAmount) < 0.01
        );

        if (linkedOrderItemIndex === -1) {
          return res.status(409).json({
            success: false,
            message: 'The linked order could not be synchronized safely. Item was not removed.',
          });
        }
      }
    }

    // Remove the item from the session sub-order.
    foundItem.deleteOne();
    session.subOrders = (session.subOrders || []).filter((sub) => sub.items.length > 0);

    // Flatten the surviving session items and recalculate all financial fields server-side.
    ...
    const customer = session.customerId
      ? await Customer.findById(session.customerId).select('state')
      : null;
    const taxType = determineTaxType(franchise.state || '', customer?.state || franchise.state || '');
    const tax = calculateOrderTax(remainingItems, taxType);

    session.subtotal = tax.subTotal;
    ...
    if (linkedOrder && linkedOrderItemIndex >= 0) {
      linkedOrder.items.splice(linkedOrderItemIndex, 1);
      const orderTax = calculateOrderTax(
        ...
        linkedOrder.tax_type || taxType
      );
      linkedOrder.sub_total = orderTax.subTotal;
      linkedOrder.cgst_amount = orderTax.cgst;
      linkedOrder.sgst_amount = orderTax.sgst;
      linkedOrder.igst_amount = orderTax.igst;
      linkedOrder.total_tax = orderTax.totalTax;
      linkedOrder.gross_total = orderTax.grossTotal;
      linkedOrder.final_amount = +(orderTax.grossTotal - Number(linkedOrder.total_discount || linkedOrder.discount_amount || 0) - Number(linkedOrder.coupon_discount || 0)).toFixed(2);
    ...
    // Audit log. Never record the PIN itself or its hash.
    try {
      const { logAudit } = require('../utils/auditHelper');
      await logAudit({
        action: 'ORDER_ITEM_REMOVED',
        userId: req.user._id,
        franchiseId: sessFranchise,
        details: {
          sessionId: session._id,
          ...
          quantity: itemQty,
          amount: itemAmount,
          reason: reason || 'Manual void',
        },
      });
    } catch (auditErr) {
      console.warn('[removeSessionItem] audit log failed:', auditErr.message);
    }

    // Let other franchise screens refresh their session/order state.
    const io = req.app.get('io');
    if (io && sessFranchise) {
      io.to(`franchise:${sessFranchise}`).emit('session:itemRemoved', {
        sessionId: session._id,
        itemId,
        itemName,
        orderId: linkedOrder?._id || foundSubOrder.order_id || null,
