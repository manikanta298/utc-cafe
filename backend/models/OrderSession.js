...
const sessionItemSchema = new mongoose.Schema({
  menuItemId: { type: mongoose.Schema.Types.ObjectId, ref: 'MenuItem' }, name: { type: String, required: true }, qty: { type: Number, required: true, min: 1 },
  unitPrice: { type: Number, required: true }, totalPrice: { type: Number, required: true }, gst_rate: { type: Number, default: 5 }, hsn_code: { type: String, default: '' }, notes: { type: String, default: '' },
}, { _id: true });
...
const paymentEntrySchema = new mongoose.Schema({
  amount: { type: Number, required: true }, method: { type: String, enum: ['Cash','UPI','Card','Net Banking','Wallet','Loyalty Points','Split'], required: true }, paidAt: { type: Date, default: Date.now }, receivedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'User' }, reference: { type: String, default: '' },
}, { _id: true });
const orderSessionSchema = new mongoose.Schema({
  ...
  loyaltyPointsRedeemed: { type: Number, default: 0 }, loyaltyRedemptionAmount: { type: Number, default: 0 }, loyaltyPointsEarned: { type: Number, default: 0 },
  paidAmount: { type: Number, default: 0 }, paymentStatus: { type: String, enum: ['unpaid','partially_paid','advance_paid','fully_paid'], default: 'unpaid' }, payments: [paymentEntrySchema], invoiceId: { type: mongoose.Schema.Types.ObjectId, ref: 'Invoice', default: null },
...
orderSessionSchema.index({ franchiseId: 1, tokenNumber: 1 }, { unique: true, sparse: true, partialFilterExpression: { tokenNumber: { $type: 'string' } }, name: 'franchiseId_tokenNumber_partial_unique' });
